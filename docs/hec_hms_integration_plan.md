...
第一阶段 MVP 已实现：诊断 HEC-HMS/Java/可执行入口，并从 HydroLite/QGIS/Watershed 生成 basin/met/control/run、映射表、manifest、报告和安全运行 stub。当前状态为 `partial`，`runnable_status` 为 `unverified`。详见 `docs/hec_hms_project_generator.md`。

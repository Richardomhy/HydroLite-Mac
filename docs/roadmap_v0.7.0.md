...
9. HEC-HMS 项目生成 MVP：环境探测、HydroLite/QGIS/Watershed 映射、项目骨架、manifest 和复核报告；真实运行仍未验证。
...
- HEC-HMS 不可用或命令行未验证时，仍可生成和校验 `unverified` 项目骨架。
- 轻量率定输出参数、指标、图表和报告。
- 报告模板能生成可交付的中文示例报告。
...
- HEC-HMS 项目生成和流域划分仅以 MVP `partial` 开放；HEC-HMS 真实运行、洪水/干旱预测和用户手册导出未实现前保持 planned。

...
def _validate_observed(case_file: Path, case_name: str, raw: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    observed = raw.get("observed", {}) or {}
    if not isinstance(observed, dict) or not observed.get("enabled", False):
        _add(rows, case_file, case_name, "observed", "enabled", "passed", "Observed streamflow is not enabled.", "info")
        return
    csv_value = observed.get("observed_streamflow_csv")
    time_col = observed.get("time_column", "datetime")
    flow_col = observed.get("flow_column", "observed_streamflow_m3s")
    gauge_col = observed.get("gauge_id_column", "gauge_id")
    if not csv_value:
        _add(rows, case_file, case_name, "observed", "observed_streamflow_csv", "failed", "observed_streamflow_csv is required.", "fatal")
        return
    path = _resolve(_base_dir(case_file), csv_value)
    if not path.exists():
        _add(rows, case_file, case_name, "observed", "file_exists", "failed", f"Observed streamflow CSV not found: {path}", "fatal")
        return
    _add(rows, case_file, case_name, "observed", "file_exists", "passed", f"Found {path}.", "info")
    try:
        df = pd.read_csv(path)
    except Exception as exc:
        _add(rows, case_file, case_name, "observed", "read", "failed", f"Cannot read observed streamflow CSV: {exc}", "fatal")
        return
    for label, column in [("time_column", time_col), ("flow_column", flow_col), ("gauge_id_column", gauge_col)]:
        if column not in df.columns:
            _add(rows, case_file, case_name, "observed", label, "failed", f"Missing observed column: {column}", "fatal")
        else:
            _add(rows, case_file, case_name, "observed", label, "passed", f"Observed column found: {column}", "info")
    if time_col in df.columns:
        ...
        _add(
            ...
            "observed",
            "time_parse",
            "failed" if parsed.isna().any() else "passed",
            f"Column {time_col} contains unparseable times." if parsed.isna().any() else f"Column {time_col} can be parsed.",
            "fatal" if parsed.isna().any() else "info",
        )
    if flow_col in df.columns:
        flow = pd.to_numeric(df[flow_col], errors="coerce")
        bad = flow.isna().any() or (flow < 0).any()
        _add(
            rows,
            case_file,
            case_name,
            ...
            f"Column {flow_col} must be numeric and non-negative." if bad else "Observed streamflow values are non-negative.",
            "fatal" if bad else "info",
        )
...
def _validate_one(case_file: Path) -> list[dict[str, Any]]:
    ...
    _validate_observed(case_file, case_name, raw, rows)
    return rows

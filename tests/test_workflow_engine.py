...
def test_workflow_engine_import_and_stage_schema():
    ...
    assert stages
    assert {"application_runtime", "project_operations", "run_orchestration", "artifact_management", "deployment_readiness", "workspace", "data_center", "data_acquisition", "data_standardization", "model_input_build", "hec_hms_project", "flood_forecast", "drought_forecast", "desktop_build", "desktop_signing", "desktop_packaging", "desktop_notarization", "desktop_update_readiness"} <= {stage["stage_id"] for stage in stages}
    assert all(next(stage for stage in stages if stage["stage_id"] == name)["status"] == "partial" for name in ("application_runtime", "project_operations", "run_orchestration", "artifact_management", "deployment_readiness", "data_center", "data_acquisition", "data_standardization", "model_input_build"))
    ...
    assert all(next(stage for stage in stages if stage["stage_id"] == name)["status"] == "partial" for name in ("desktop_build", "desktop_signing", "desktop_packaging", "desktop_notarization", "desktop_update_readiness"))
